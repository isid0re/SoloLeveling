...
var Performance = {
	...
	set: function() {
		this.create();
		if(me.gametype === 0){
			var files = ["den", "bloodraven", "mausoleum", "tristam", "countess", "countessMF", "pits", "andariel", "andarielMF",
	...
	updateStats: function(name, arg, value) {
		...
		var diff, i, obj, string,
			statArr = [];
		var tt, ta, fsa, fla, fa, uftt, sa, la, a, cl;
		...
		for (i = 0; i < statArr.length; i += 1) {
			...
			case "TotalTime":
				...
				obj[diff][name].AVG = (uftt)/(obj[diff][name].TotalAttempts); //Want to know average time
