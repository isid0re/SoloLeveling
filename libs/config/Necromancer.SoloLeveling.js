...
function LoadConfig () {
	...
	var levelingGear = [ // autoequip setup
		//weapon
		"([type] == wand || ([type] == sword || [type] == mace) && ([Quality] >= Magic || [flag] == runeword) || [type] == knife && [Quality] >= Magic) && [flag] != ethereal # [secondarymindamage] == 0 && [itemchargedskill] >= 0 # [tier] == tierscore(item)",
		//Helmet
	...
	if (!me.classic) {
		if (!Check.haveItem("wand", "runeword", "White")) {
			var white = [
				...
				"[type] == wand && [flag] != runeword && [Quality] >= Normal && [Quality] <= Superior # [poisonandboneskilltab] + [skillbonespear] >= 2 && [fcr] == 20 && [Sockets] == 2 # [MaxQuantity] == 1"
			];
		...
		if ((me.ladder || Developer.addLadderRW) && Item.getEquippedItem(4).tier < 777) { // Spirit Sword
			...
			NTIP.addLine("([Name] == BroadSword || [Name] == CrystalSword) && [flag] != runeword && [flag] != ethereal && [Quality] >= Normal && [Quality] <= Superior # [Sockets] == 4 # [MaxQuantity] == 1", "Runeword Spirit");
