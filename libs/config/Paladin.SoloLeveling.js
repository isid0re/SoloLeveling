	...
	if (!me.classic) {
		...
		if (!Check.haveItem("armor", "runeword", "Enigma") && ["Hammerdin", "Smiter"].indexOf(SetUp.finalBuild) > -1) { // Enigma - Don't make if not Smiter or Hammerdin
			var Enigma = [
				...
				"[Name] == BerRune",
				"([Name] == MagePlate || [Name] == ScarabHusk || [Name] == WyrmHide || [Name] == DuskShroud) && [Flag] != Ethereal && [Quality] >= Normal && [Quality] <= Superior # [Sockets] == 3 # [MaxQuantity] == 1",
			];
		...
		if (Item.getEquippedItemMerc(3).prefix !== 20547 && ["Hammerdin", "Smiter"].indexOf(SetUp.finalBuild) > -1) { // Merc Fortitude
			var fort = [
				"[Name] == ElRune # # [MaxQuantity] == 1",
				"[Name] == SolRune # # [MaxQuantity] == 1",
				"[Name] == DolRune # # [MaxQuantity] == 1",
